...
namespace NPBehave
{
    public class NavMoveTo : Task
    {
        ...
        private void onBlackboardValueChanged(Blackboard.Type type, NPValueBase key)
        {
            moveToBlackboardKey();
        }
        ...
        private void moveToBlackboardKey()
        {
			...
			var target = Blackboard.Get< Transform>(blackboardKey);
			// get target location
			...
			{
				var target1 = Blackboard.Get<Vector3>(blackboardKey);
				if (target1 != null)
				{
					destination = target1;
				}
