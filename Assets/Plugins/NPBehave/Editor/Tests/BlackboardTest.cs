...
#pragma warning disable 618 // deprecation
    ...
    public class BlackboardTest
    {
        ...
        {
            bool notified = false;
            this.sut.AddObserver("test", ( Blackboard.Type type, NPValueBase value ) =>
            {
                notified = true;
            });
        ...
        {
            bool notified = false;
            System.Action<Blackboard.Type, NPValueBase> obs1 = null;
            System.Action<Blackboard.Type, NPValueBase> obs2 = null;

            obs1 = ( Blackboard.Type type, NPValueBase value ) =>
            {
                Assert.IsFalse(notified);
                notified = true;
                this.sut.RemoveObserver("test", obs2);
            };
            obs2 = ( Blackboard.Type type, NPValueBase value) =>
            {
                Assert.IsFalse(notified);
                notified = true;
        ...
        {
            this.sut.Set("test", 1f);
            Assert.AreEqual(this.sut.Get<float>("test"), 1f);
            this.sut.Unset<float>("test");
            this.sut.Set("test", "something");
            Assert.AreEqual(this.sut.Get<string>("test"), "something");
        }
        ...
        public void ShouldListenToEvents_WhenUsingChildBlackboard()
        {
            ...
            var firstCondition = new BlackboardCondition("branch1", Operator.IS_EQUAL, NPValueFactory.Create(true), Stops.IMMEDIATE_RESTART, firstChild);
            var secondCondition = new BlackboardCondition("branch2", Operator.IS_EQUAL, NPValueFactory.Create(true), Stops.IMMEDIATE_RESTART, secondChild);
