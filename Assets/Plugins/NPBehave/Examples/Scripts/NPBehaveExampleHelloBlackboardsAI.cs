...
public class NPBehaveExampleHelloBlackboardsAI : MonoBehaviour
{
	...
	void Start()
	{
		...
		var node = new BlackboardCondition("foo", Operator.IS_EQUAL,NPValueFactory.Create(true), Stops.IMMEDIATE_RESTART, new Sequence(new Action(() => Debug.Log("foo")), new WaitUntilStopped()));
		var node2 = new Sequence(new Action(() => Debug.Log("bar")), new WaitUntilStopped());
		Selector selector = new Selector(node ,node2);
		Service service = new Service(0.5f, () => { behaviorTree.Blackboard.Set("foo",!behaviorTree.Blackboard.Get<bool>("foo")); }, selector);
		behaviorTree = new Root(service);
		behaviorTree.Start();

		var debug = gameObject.AddComponent<Debugger>();
		debug.BehaviorTree = behaviorTree;
	}
	/*
    void Start()
    {
